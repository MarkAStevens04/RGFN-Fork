...
`git status --short` shows the new files under `experiments/oracle_validation/aizynth_failure_modes/`,
this log, and the `docs/RESEARCH_CONTEXT.md` index row as untracked/modified on branch
`Hub-Analysis`. **Not yet committed.** Can you commit the experiment files
(`experiments/oracle_validation/aizynth_failure_modes/`, `Logs/047_*.md`, `docs/RESEARCH_CONTEXT.md`)?
Once you do, let me know and I'll fill in the commit hash here. `[TODO — add commit hash after pushing]`
The stock hdf5s / SMILES lists / per-run JSON are large scratch artifacts, regenerated by
`build_block_stock.py` + the cache, and are intentionally not committed.

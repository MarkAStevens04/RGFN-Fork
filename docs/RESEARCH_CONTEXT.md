...
| [025](../Logs/025_lsdflow-phase1-hub-extraction-seh.md) | 2026-07-08 | sEH / RGFN — LSD-Flow phase 1: post-hoc hub extraction from the trained flow field | **Machinery built + validated end-to-end; batchable hubs exist, with severe-test caveats.** New LSD-Flow acquisition primitives (`glue/metrics/lsdflow_flow.py`+`uncertainty.py`, `glue/samplers/lsdflow/**` = flow recovery + `U(h)` + 6 hub / 3 molecule strategies + `LSDFlowAcquisition`) and analysis world (`validation/lsdflow/**` = RGFN adapter, canonical `HubDAG`, Butina diversity + reactions-per-mode cost, harness), split across the two axes per `docs/LSD_FLOW_PROPOSAL.md`. Ran on the trained RGFN sEH stdlib model (entry `020`, job 69616, epoch 5001; **caught + rejected two cancelled 30/40-iter sibling stubs first**). On 10k sampled trajectories: 613/8183 hubs multi-child (max 9); hub-amortized library selection cuts reactions-per-mode ~40% (3.55 vs 6.24) — **but** 91% of multi-child hubs sit at the `max_num_reactions` boundary (forced stop), flow ranking does **not** beat the `parent_of_topk` control on cost (3.55 vs 3.47), and the flow-vs-visitation TB-integrity correlation is near zero (0.086). Next: control the N confound, split boundary vs interior hubs, run the severe-test suite, add phase-2 `enumerate_children`. Not committed (branch Hub-Analysis). |

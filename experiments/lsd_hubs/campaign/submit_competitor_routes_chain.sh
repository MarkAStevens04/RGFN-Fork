...
#
# RUN_SB=1 additionally runs the SPARROW-Batching arm (slow, and its middle budgets hit the MILP
# wall — see submit_competitor_routes.sh). Default is greedy only.
...
for CELL in $CELLS; do
    ...
    GENERATOR="$GEN" TARGET="$TGT" SEED="$SD" RUN_DIR="$RUN_DIR" \
